...
def gen_top_10():
    ...
    results = sorted([r for r in results if r.batch==2018 and r.image],
    ...
    #draw heading
    d.text((MARGIN_LEFT + 170,MARGIN_TOP), "Results", font=FONT_HEAD, fill=FONT_COLOR_HEAD)

    for i, r in enumerate(results[:11]):
        r.image.thumbnail(THUMB_SZIE)

        # student rank
        ROW_Y = MARGIN_TOP + FONT_HEAD.size + MARGIN_HEAD_ROW + i*(THUMB_SZIE[1] + MARGIN_ROW)
        d.text((MARGIN_LEFT, ROW_Y + THUMB_SZIE[1]/4), f"{i + 1}.", fill=FONT_COLOR, font=FONT)

        # student image
        RANK_MAX_SIZE = d.textsize("99.", font=FONT)
        THUMB_X = MARGIN_LEFT + RANK_MAX_SIZE[0] + MARGIN_RANK_THUMB
        base.paste(r.image, (int(THUMB_X), int(ROW_Y), int(THUMB_X + r.image.size[0]), int(ROW_Y + r.image.size[1])))

        # student name & cgpa
        NAME_X = THUMB_X + THUMB_SZIE[0] + 20
        d.text((NAME_X, ROW_Y + THUMB_SZIE[1]/4), f"{r.student_name}", fill=FONT_COLOR, font=FONT)
        d.text((CGPA_X, ROW_Y + THUMB_SZIE[1]/4), f"{r.cgpa}", fill=FONT_COLOR, font=FONT)

    # watermark
    watermark = Image.new("RGBA", base.size, (255,255,255,0))
    dw = ImageDraw.Draw(watermark)
    dw.text((SIZE[0]/4, SIZE[1]/2 - FONT_HEAD.size), WATERMARK_TXT[0], font=FONT_HEAD, fill=(0,0,0,64))
    dw.text((SIZE[0]/4 + 60, SIZE[1]/2 + 10), WATERMARK_TXT[1], font=FONT_HEAD, fill=(0,0,0,64))
    dw.text((SIZE[0]/4 + 120, SIZE[1]/2 + 10 + FONT_HEAD.size), WATERMARK_TXT[2], font=FONT, fill=(0,0,0,64))
